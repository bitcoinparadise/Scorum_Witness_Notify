...
// Connect to the specified RPC node (Default: https://prodnet.scorum.com)
rpc_node = config.rpc_nodes ? config.rpc_nodes[0] : (config.rpc_node ? config.rpc_node : 'https://prodnet.scorum.com');
scorum.api.setOptions({ url: rpc_node});
scorum.config.set(config.chain_id);
    ...
    try {
        ...
        while(true) {
            ...
            if (missedCount == -1)
            {
               ...
               try {
                    let witness = await scorum.api.getWitnessByAccountAsync(config.accountname);
